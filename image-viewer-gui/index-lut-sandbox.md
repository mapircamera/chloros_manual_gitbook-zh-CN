...
2. 鼠标悬停查看可用渐变预设
3. 选择所需渐变
4. 勾选索引框后图像将**即时更新**新配色

{%提示 style=&quot;success&quot; %}
**最佳实践**：对于植被指数（如NDVI），Red-黄-Green渐变最直观，因其符合自然色彩认知（绿=健康，黄=中等，红=受胁）。
{% endhint %}
...
* **少分类**（3-5）：形成鲜明区域，简化分类，易于区分类别
* **中等分类**（6-10）：平衡方案，适用于多数场景
* **多类别**（15-20）：平滑过渡，精细差异，呈现照片级效果
...
* **增强**目标区域的**对比度**
* **排除无关值**（如水体、裸露土壤）
* **标准化**多幅图像或不同时段的**可视化效果**
* **突出显示**窄值域内的**细微差异**

### 剪裁超出范围的值

当像素值超出定义的最小/最大范围时，可通过**剪裁模式**控制其显示方式。

#### **可用剪裁模式选项：**

#### 1. 最小值与最大值

* **低于最小值**的像素 → 使用渐变色系的**首色**显示（如红色）
...
| **透明背景** | GIS叠加层，特定范围隔离    | 范围着色，超出区域留白 |
| **指数背景**       | 细微强调，保持数据关联性  | 范围着色，超出区域灰化  |
| **原始背景**    | 报告演示，直观分析支持 | 范围着色，超出区域保留照片 |

### 创建自定义LUT颜色

为实现可视化效果的完全掌控，您可通过编辑单个色彩停点创建**自定义色彩渐变**。
...
{%提示 style=&quot;success&quot; %}
**实时反馈**：即时视觉反馈让您能快速尝试不同设置，直至找到满足分析需求的最佳可视化方案。
{%结束提示 %}
...
{% 提示 style=&quot;warning&quot; %}
**公式验证**：确保公式使用的波段在相机中可用。例如，RedEdge仅适用于配备RedEdge滤光片的相机。
{% endhint %}
...
* [**全屏打开图像**](opening-an-image-full-screen.md) - 图像查看器基础操作
