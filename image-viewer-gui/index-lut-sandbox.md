...
在Chloros中，索引可在项目处理阶段应用。要确定导出时所需的索引与LUT设置，最便捷的方式是使用图像查看器沙盒。
...
完整指数公式文档参见[多光谱指数公式](../project-settings/multispectral-index-formulas.md)。
...
{%提示 style=&quot;success&quot; %}
**最佳实践**：对于植被指数（如NDVI），Red-Yellow-Green渐变最直观，因其符合自然色彩关联（绿色=健康，黄色=中等，红色=受胁）。
{% endhint %}
...
* **中等类数** (6-10): 平衡方案，适用于多数场景
* **多类数** (15-20): 平滑过渡，精细变化，呈现照片级效果

**适用场景：**

* **少类数 (3-5)**: 演示文稿、分类地图、简易报告
* **中等类别（6-10）**：通用分析、均衡细节、标准报告
* **大量类别（15-20）**：科学分析、精细检测、出版级输出

### 精细调整数值范围

**数值范围控制**决定渐变色中各索引值对应的颜色：
...
{% 提示 style=&quot;info&quot; %}
**自动缩放**：初次应用LUT时，Chloros会自动将最小/最大值设为图像实际数据范围。您可随后缩小该范围以聚焦特定目标值区间。
{% endhint %}
...
{% 提示 style=&quot;info&quot; %}
**创建位置**：自定义指数可在处理前的**项目设置**中配置，也可在图像查看器沙盒侧边栏中设置。
{% endhint %}
