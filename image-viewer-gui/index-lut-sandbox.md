...
**分级控制**决定渐变中出现的离散色阶数量：**分级数量选项：*** **2-5级**：大范围分类，分区明显
* **6-10级**：平衡适中，分类效果佳
* **11-20级**：平滑过渡，视觉连续
* **20+级**：近乎连续，最大平滑度**调整方法：**

1. 在LUT面板中，定位**渐变条下方的色块区域**

2. 通过+按钮增加类数
3. 双击色块可减少类数
4. 渐变效果将**实时**更新于图像**可视化效果：*** **少级（3-5级）**：形成鲜明分区，简化分类，易于辨识类别
* **中级（6-10级）**：平衡方案，适用于多数场景
* **多级（15-20级）**：平滑过渡，精细变化，呈现照片质感**适用场景：*** **少类（3-5类）**：演示幻灯片、分类地图、简易报告
...
**范围调整示例：*** **全范围**：`-1.0` 至 `1.0` （显示所有可能值）
* **植被聚焦**：`0.2` 至 `0.9`（排除裸土与水体）
* **仅健康植被**：`0.5` 至 `0.9` （仅突出显示旺盛植被）
* **胁迫检测**：`0.2` 至 `0.5`（强调问题区域）
* **自定义范围**：根据观测像素值调整**为何调整范围？*** **增强**目标区域的**对比度*** **排除无关数值**（如水体、裸露土壤）
* **标准化**多张图像或不同时段的**可视化效果*** **突出显示**窄值域内的**细微差异**
...
* **在范围内**的像素呈现**色彩渐变*** **应用场景**：细微高亮处理，在保持上下文的同时突出重点区域
...
| **索引背景**       | 微调强调，保持数据关联   | 范围着色，超出灰度 |
| **原始背景**     | 报告演示，直观分析         | 范围着色，超出照片 |

### 创建自定义LUT颜色

为实现可视化效果的完全掌控，您可通过编辑单个颜色停驻点创建**自定义颜色渐变**。**创建自定义渐变步骤：**
...
**添加或删除颜色停靠点：*** **添加色阶点**：点击+图标在末尾添加新色块
* **删除色阶点**：双击色块即可移除**定制策略：*** **反转渐变**：颠倒颜色顺序以逆转含义（例如绿=低，红=高）
* **品牌色**：匹配机构报告色板
* **色盲友好**：采用橙蓝或紫黄配色组合
* **打印优化**：选择同时适用于彩色与灰度打印的颜色
* **多阈值**：在特定数值阈值处使用差异化颜色进行分类

{% 提示 style=&quot;info&quot; %}
**保存自定义渐变**：可保存并复用自定义渐变。点击LUT面板中的保存图标，即可将自定义配色方案留存供后续使用。
{% endhint %}
...
* **更改类** → 渐变平滑度立即更新
* **修改剪切** → 背景显示即时变化
* **编辑颜色** → 自定义渐变立即生效**无需&quot;应用&quot;按钮** - 所有变更皆实时交互！
...
5. **记录**不同特征的数值范围：
   ...
   * **水体**（若存在）：例如 NDVI -0.05 至 0.10**使用像素值设定LUT范围：**检查像素值后，相应调整LUT的最小/最大值：**示例场景：*** **观测结果**：土壤值 = 0.05-0.25，受胁值 = 0.25-0.50，健康值 = 0.50-0.85
* **目标**：仅可视化植物健康状态（排除土壤）
* **LUT设置**：最小值=`0.25`，最大值=`0.85`
* **裁剪**：&quot;原始背景&quot;模式以自然色呈现土壤
* **结果**：色彩渐变仅作用于植被，土壤呈现原始图像

{% hint style=&quot;info&quot; %}
**动态范围**：不同作物、季节及生长阶段将呈现不同数值区间。设置LUT范围前务必核查特定数据集的像素值。
{% endhint %}
...
* **批量处理**：将优化索引应用于完整数据集
* **深入了解**：阅读[多光谱索引公式](../project-settings/multispectral-index-formulas.md)
