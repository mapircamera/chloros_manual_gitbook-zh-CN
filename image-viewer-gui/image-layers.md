...
* 示例：原始数据（NDVI 指数）、原始数据（NDRE 指数）、原始数据（GNDVI 指数）等
* 显示指数计算结果的单波段灰度图像
* 项目设置中配置的每个指数对应一层
...
* **统一工作流**：选定单一图层（如NDVI）完成全数据集审查后再切换
...
* **索引图层**：加载速度与反射率图层相当
* **首次加载最慢**：后续查看同一图层时缓存机制将提升速度

### 质量验证

* **务必检查原始RAW数据**：在信任处理结果前验证源数据质量
* **对比图层**：通过切换图层验证处理是否正确
...
**解决方案**：当首选图层不可用时，Chloros会自动回退至JPG格式——此为正常行为
