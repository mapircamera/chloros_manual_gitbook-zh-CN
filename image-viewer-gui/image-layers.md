...
* **反射率校正**输出（若启用反射率校准）
* **目标图像**（若图像包含校准目标）
* **索引图像**（若配置索引，则生成NDVI、NDRE、GNDVI等）
...
**16位TIFF处理方式：**除以65,535获取百分比反射率 **32位百分比处理方式：**数值直接代表百分比（0.5=50%反射率）
